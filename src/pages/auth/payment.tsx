...
const PaymentPage: React.FC = () => {
    ...
    // Initialize payment from location state or storage
    useEffect(() => {
        const state = location.state as any;
        
        // Priority 1: Jika ada payment di location.state, gunakan itu
        if (state?.payment && !payment) {
            setPaymentFromState(state.payment);
            return;
        }
        
        // Priority 2: Atau coba load dari storage
        if (!payment) {
            loadPaymentFromStorage();
        }
        
        // Set timeout to trigger fallback if payment not loaded in 3 seconds
    ...
    // Redirect when payment successful
    useEffect(() => {
        if (isPaid) {
            setTimeout(() => {
                clearPayment();
    ...
    const handlePayNow = () => {
        ...
        // Start polling after payment window opens
        setTimeout(() => {
            startPolling();
        }, 1500);
    };
        ...
        return (
            ...
            >
                <Box style={{ textAlign: "center", maxWidth: 400 }}>
                    ...
                    {/* Emergency Help after 5 seconds */}
                    {loadTimeout && (
                        <Alert
                            icon={<IconX size={18} />}
                            color="yellow"
