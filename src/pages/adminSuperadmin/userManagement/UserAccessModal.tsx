...
import type { UserDetail, UpdateUserAccessRequest, AvailableMenu, AvailablePermission } from '../../../api/admin';
...
export function UserAccessModal({
  ...
  // Sync form with user data and available menus/permissions
  useEffect(() => {
    if (user && availableMenus.length > 0) {
      setRole(user.role);
      
      // Initialize menu updates from availableMenus (which has isEnabled from backend)
  ...
  const handleSave = async () => {
    // Build menu access updates (only send changed ones or all enabled)
